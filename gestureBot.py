# Computer Vision Algorithm Reference: https://gogul.dev/
...
def segment(image, threshold=25):
    ...
    (cnts, _) = cv2.findContours(
        thresholded.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
...
count_data = []  # list to store the number of fingers' history
    ...
    while True:
        ...
        if num_frames < 30:
            ...
            if hand is not None:
                ...
                # time.sleep(2)
                # if(time.time() - time_counter >= 2):
                # fingers_list.clear()
                # fingers_list.append(0)
                # time_counter=time.time()
                # print(fingers_list)

                # lk = statistics.mode(fingers_list)
                # print(lk)
                ...
                count_data.append(fingers)

                if keypress == ord("q"):
                    break

                # Communications:
                if len(count_data) % 30 == 0:
                    print("Fingers: " + str(count_data[-1]))
                    aio.send("gesturebot", count_data[-1])
